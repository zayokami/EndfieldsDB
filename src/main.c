...
static void test_index_auto_rehash(void)
{
    ...
    uint32_t i;
    const uint32_t n_keys = 300;
    char key[24];

    printf("\n=== v4: index auto rehash on load threshold ===\n");

    err = ef_open_memory_hash(arena, sizeof(arena), 512, 16, 1, &db);
    ...
    for (i = 0; i < n_keys; ++i) {
        snprintf(key, sizeof(key), "auto-%04u", i);
        err = ef_alloc(db, &slot_id);
        expect_err(err, EF_OK, "alloc for auto rehash");
        if (err != EF_OK) {
            break;
        }
        if (i == 0) {
        ...
        err = ef_write_payload(db, slot_id, key, (uint8_t)strlen(key));
        expect_err(err, EF_OK, "write payload for auto rehash");
        err = ef_index_put(db, key, slot_id);
        expect_err(err, EF_OK, "auto-rehash put must never fail with INDEX_FULL");
        if (err != EF_OK) {
            break;
        }
    }

    expect_true(ef_index_capacity(db) > 16, "capacity grew past initial 16");
    expect_true(ef_index_capacity(db) <= EF_INDEX_MAX_CAPACITY, "capacity within max");
    expect_true(ef_index_count_entries(db) == n_keys, "entry count matches inserted keys");

    /* Load factor must stay below the configured threshold after auto growth. */
    expect_true((uint64_t)ef_index_count_entries(db) * EF_INDEX_REHASH_LOAD_FACTOR_DEN <=
                    (uint64_t)ef_index_capacity(db) * EF_INDEX_REHASH_LOAD_FACTOR_NUM,
                "load factor under threshold after growth");
    ...
    /* Overwriting an existing key must not change the entry count. */
    {
        uint32_t count_before = ef_index_count_entries(db);
        snprintf(key, sizeof(key), "auto-%04u", 0U);
        err = ef_index_put(db, key, first_slot);
        expect_err(err, EF_OK, "overwrite existing key");
        expect_true(ef_index_count_entries(db) == count_before, "overwrite keeps entry count");
        err = ef_index_get(db, key, &looked);
        expect_err(err, EF_OK, "overwritten key still found");
        expect_true(looked == first_slot, "overwritten key maps to same slot");
    }

    /* Removing keys must decrement the entry count. */
    {
        uint32_t count_before = ef_index_count_entries(db);
        snprintf(key, sizeof(key), "auto-%04u", 1U);
        err = ef_index_remove(db, key);
        expect_err(err, EF_OK, "remove key");
        expect_true(ef_index_count_entries(db) == count_before - 1U, "remove decrements count");
        expect_err(ef_index_get(db, key, &looked), EF_ERR_NOT_FOUND, "removed key not found");
    }
...
static void run_hash_perf_suite(volatile uintptr_t *sink)
{
    ...
    for (r = 0; r < BENCH_ROUNDS; ++r) {
        ...
        err = ef_open_memory_hash(hash_arena, sizeof(hash_arena), 128, 16, 1, &db);
        if (err != EF_OK || db == NULL) {
            break;
        }
